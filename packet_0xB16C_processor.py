...
class Packet_0xB16C:
    def __init__(self, packet_text, config, entry):
        ...
        self.config = config
        self.entry = entry
        self.pattern1 = r".*?Subscription ID = (?P<Subs_ID>[\d]+).*?Duplex Mode = (?P<duplex_mode>[\w]+).*?Subframe Cfg = (?P<subframe_cfg>[\d]+).*?Number of Records = (?P<no_of_records>[\d]+).*?"
        self.pattern2 = r".*?\|Level\s+\|Space\s+\|.*?-+.*?\n(?P<table>[\s\S]*)\n\n"
        self.dict = {}
        self.result = []
    ...
    def table_pattern(self):
        ...
        if match:
            data = table_config(match, self.config['DCI Info Records'], self.config)
            return data
        else:
