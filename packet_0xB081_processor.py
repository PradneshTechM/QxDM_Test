...
class Packet_0xB081:
    def __init__(self, packet_text, config, entry):
        ...
        self.config = config
        self.entry = entry
        self.pattern1 = r".*?Subscription ID = (?P<Subs_ID>[\d]+).*?Submit Reason = (?P<reason>[\w\_\w]+).*?Config Mask = (?P<config_mask>[\w\_\w\s\|\s]+)\n.*?"
        self.pattern2 = r".*?\|\(ms\)\s*\|Length\|.*?-+.*?\n(?P<table>[\s\S]*)\n\n"
    ...
    def table_pattern(self):
        ...
        if match:
            data = table_config(match, self.config['RB Config'], self.config)
            return data
        else:
