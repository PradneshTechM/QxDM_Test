...
class Packet_0xB0B5:
    def __init__(self, packet_text, config, entry):
        ...
        self.config = config
        self.entry = entry
        self.pattern1 = r".*?Subscription ID = (?P<Subs_ID>[\d]+).*?SRB Ciphering Keys \(hex\) = (?P<srb_cipher>.*?(?=\n)).*?SRB Integrity Keys \(hex\) = (?P<srb_integrity>.*?(?=\n)).*?"
    ...
    def table_pattern(self):
        ...
        if match:
            data = table_config(match, self.config['Entry'], self.config)
            return data
        else:
