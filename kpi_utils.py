...
def simple_map_entry(data, config):
    ...
    for i, (config_key, config_value) in enumerate(config.items(), start=0):
        if i < len(data_values):
            mapped_value = data_values[i - 1]  # Use original value, adjusted for index starting at 0
            ...
            if 'Field Type' in config_value:
                if config_value['Field Type'] == 'Positive Number':
                    ...
                    mapped_value = float(mapped_value)  # Assuming you import or define Decimal
...
def map_entry(data, config):
    ...
    for i, (config_key, config_value) in enumerate(config.items(), start=0):
        if i < len(data_values):
            mapped_value = data_values[i - 1]  # use original value
