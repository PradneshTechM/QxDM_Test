...
class Packet_0xB168:
    def extract_info(packet_text, config=None, entry=None):
        ...
        pattern = r'.*?Subscription ID = (?P<Subs_ID>[\d]+).*?Cell Index = (?P<Cell_Index>[\d]+).*?RACH Procedure Type = (?P<RACH_Type>[a-zA-Z]+).*?RACH Procedure Mode = (?P<RACH_Mode>[a-zA-Z]+).*?RNTI Type = (?P<RNTI_Type>[a-zA-Z_]+).*?Timing Advance Included = (?P<Timing_Advance_Included>[a-zA-Z]+).*?Timing Advance = (?P<Timing_Adance>[\d]+)'
