...
class Packet_0xB0B1:
    def __init__(self, packet_text, config, entry):
        ...
        self.config = config
        self.entry = entry
        self.pattern1 = r".*?Subscription ID = (?P<Subs_ID>[\d]+).*?Num RB ID Configured = (?P<num_rb_id>[\d]+).*?"
        self.pattern2 = r".*?\|#\s+\|PDCP Payload\s+\|.*?-+.*?\n(?P<table>[\s\S]*)\n\n\n"
        self.dict = {}
        self.result = []
    ...
    def table_pattern(self):
        ...
        if match:
            data = table_config(match, self.config['PDUs'], self.config)
            return data
        else:
