...
class Packet_0xB0A1:
    def __init__(self, packet_text, config, entry):
        ...
        self.config = config
        self.entry = entry
        self.pattern1 = r".*?Subscription ID = (?P<Subs_ID>[\d]+).*?Number Of Meta = (?P<no_meta>[\d]+).*?Number Of RB = (?P<no_rb>[\d]+).*?Log Count = (?P<log_count>[\d]+).*?"
        self.pattern2 = r'.*?\|Rx Next\s+\|Next Count\s+\|.*?-+.*?\n(?P<table>[\s\S]*)\n\nMeta'
        self.dict = {}
        self.result = []
    ...
    def table_pattern(self):
        ...
        if match:
            data = table_config(match, self.config['PDCP State'], self.config)
            return data
        else:
