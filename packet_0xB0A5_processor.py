...
class Packet_0xB0A5:
    def __init__(self, packet_text, config, entry):
        ...
        self.config = config
        self.entry = entry
        self.pattern1 = r'.*?Subscription ID = (?P<Subs_ID>[\d]+).*?Subpacket ID = (?P<ID>.*?(?=\n)).*?Subpacket Size = (?P<size>.*?(?=\n)).*?Ciphering keys for SRBs \(hex\) = (?P<cipher>.*?(?=\n)).*?Integrity Keys for SRBs \(hex\) = (?P<integrity>.*?(?=\n)).*?'
        self.pattern2 = r".*?\|\(hex\)\s+\|\(hex\)\s+\|.*?-+.*?\n(?P<table>[\s\S]*)\n\n"
        self.dict = {}
        self.result = []
    ...
    def table_pattern(self):
        ...
        if match:
            data = table_config(match, self.config['DL SRB Integrity Data PDU Subpacket'], self.config)
            return data
        else:
