...
class Packet_0xB828:
    def __init__(self, packet_text, config, entry):
        ...
        self.config = config
        self.entry = entry
        self.pattern1 = r'.*?Subscription ID = (?P<Subs_ID>[\d]+).*?Source RAT = (?P<source_rat>[a-zA-Z]+).*?Current Search RAT = (?P<current_search_rat>[a-zA-Z]+).*?Network Search Status = (?P<network_search_status>[a-zA-Z]+)'
        self.pattern2 = r'.*?SCS.*?\|Band.*?\|.*?-+.*?\n(?P<table>[\s\S]*)'
        self.dict = {}
        self.result = []
    ...
    def table_pattern(self):
        ...
        if match:
            data = table_config(match, self.config['PLMN List'], self.config)
            return data
        else:
