...
class Packet_0xB064:
    def __init__(self, packet_text, config, entry):
        ...
        self.config = config
        self.entry = entry
        self.pattern1 = r".*?Subscription ID = (?P<Subs_ID>[\d]+).*?SubPacket ID = (?P<subpacket_id>[\d]+).*?"
        self.pattern2 = r".*?\|SCell2\|SCell2\s+\|.*?-+.*?\n(?P<table>[\s\S]*)\n\n"
        self.dict = {}
        self.result = []
    ...
    def table_pattern(self):
        ...
        if match:
            data = table_config(match, self.config['Uplink Transport Block V3'], self.config)
            return data
        else:
